#!/home/srini/anaconda3/envs/tfgpu/bin/checkio --domain=py run morse-decoder
...
#     for word in code:
#         for letter in word:
#             word[word.index(letter)] = MORSE[letter]
#     code = ' '.join(''.join(code[i][j] for j in range(len(code[i]))) for i in range(len(code)))
#     return code.capitalize()

def morse_decoder(code):
    return ' '.join(''.join(map(MORSE.get, word.split(' '))) for word in code.split('   ')).capitalize()
