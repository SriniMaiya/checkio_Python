#!/home/srini/anaconda3/envs/tfgpu/bin/checkio --domain=py run words-order
...
#     # your code here
#    if (len(set(words)) == len(words)) and all(word in text.split(' ') for word in words):
#       return True if all(text.index(words[i]) < text.index(words[i+1]) for i in range(len(words)-1)) else False
#    else: return False
